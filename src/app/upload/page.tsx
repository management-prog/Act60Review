...
// </div>
  ...
  className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 bg-gray-900/50 border border-gray-800 rounded-xl p-3 sm:p-4"
>
  {/* Top Row: Icon, File Details, and Mobile Delete Button */}
  <div className="flex items-center gap-3 w-full sm:flex-1 min-w-0">
    ...
    <button
      onClick={(e) => { e.stopPropagation(); removeFile(f.id) }}
      className="sm:hidden text-gray-500 hover:text-red-400 p-1 shrink-0"
    >
      <X className="w-5 h-5" />
    </button>
  </div>

  {/* Bottom Row: Dropdown & Desktop Delete Button */}
  <div className="flex items-center gap-3 w-full sm:w-auto">
    <select
      value={f.label}
      onChange={(e) => updateLabel(f.id, e.target.value)}
    ...
    {/* Desktop-only Delete Button (Far Right) */}
    <button
      onClick={(e) => { e.stopPropagation(); removeFile(f.id) }}
      className="hidden sm:block text-gray-500 hover:text-red-400 transition-colors shrink-0"
    >
      <X className="w-4 h-4" />
    </button>
  </div>
